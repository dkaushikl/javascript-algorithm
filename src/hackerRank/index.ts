export class HackerRank {
  static maximumSum(a, m) {
    ...
    for (let i = 0; i < a.length; i++) {
      ...
      for (let j = i + 1; j < a.length; j++) {
        var val = a.slice(i, j + 1).reduce((a, b) => (a += b), 0);
        console.log(val);
  ...
  static dayOfProgrammer(year) {
    ...
    let totalDays = [31, februaryDays, 31, 30, 31, 30, 31, 31];
    let sumOftotalDays = totalDays.reduce((a, b) => a + b, 0);
    let days = 256 - sumOftotalDays;
  ...
  static pageCount(n, p) {
    let bookArr: Array<Array<number>> = [];
  ...
  static repeatedString(s, n, target) {
    n = n.toLowerCase();
    let totalTarget = (s.match(/a/g) || []).length;

    let checkTargetLength = target % s.length;
    ...
    if (checkTargetLength === 0) {
      ...
      let remainingChar = (s.substr(0, checkTargetLength).match(/a/g) || []).length;
      let total = totalTarget * ((target - checkTargetLength) / s.length);
  ...
  static catAndMouse(x, y, z) {
    let firstCatSteps = Math.abs(z - x);
    let secondCatSteps = Math.abs(z - y);
  ...
  static sherLockAndValidString(s: string) {
    ...
    let result = 'yes';
    let countFirstChar = test[Object.keys(test)[0]];
  ...
  static twoLargeNumber(n1, n2) {
    ...
    if (n1.toString().length > n2.toString().length) {
      let diff = n1.toString().length - n2.toString().length;
      n2 = ''.padStart(diff, '0') + n2;
    }

    if (n2.toString().length > n1.toString().length) {
      let diff = n2.toString().length - n1.toString().length;
      n1 = ''.padStart(diff, '0') + n1;
    }

    let sum: any = 0;
    let moduler = 0;
    let n = n1.toString().length > n2.toString().length ? n1.toString().length : n2.toString().length;

    for (let i = n - 1; i >= 0; i--) {
      let first = n1.toString()[i] ? n1.toString()[i] : 0;
      let second = n2.toString()[i] ? n2.toString()[i] : 0;

      let test = Number(first) + Number(second) + Number(moduler);
  ...
  static checkSumString(n = 1111112223) {
    ...
    for (let i = 0; i < arrLength - 1; i++) {
      let newString: string = str[i] + str[i + 1];
    ...
    for (let i = 0; i < newArray.length - 1; i++) {
      for (let j = i + 1; j < newArray.length - 1; j++) {
        let sum = Number(newArray[i]) + Number(newArray[j]);
  ...
  static calculateMinMax(x1 = 645, x2 = 666) {
    let firstStrWithSix = x1.toString().replace(/5/g, '6');
    let secondStrWithSix = x2.toString().replace(/5/g, '6');

    let firstStrWithFive = x1.toString().replace(/6/g, '5');
    let secondStrWithFive = x2.toString().replace(/6/g, '5');
  ...
  static minSumOfSquares(s = 'abccc', k = 1) {
    ...
    return Object.keys(hashTable).reduce((a, b) => Math.pow(+hashTable[b], 2) + +a, 0);
  }
}
